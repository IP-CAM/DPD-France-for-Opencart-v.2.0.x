    ...
    public function dpdfranceexport()
    {
        ...
        if (isset($this->request->post['selected']))
        {
            ...
            foreach ($this->request->post['selected'] as $order_id) {
                ...
                $isops = array("DE", "AD", "AT", "BE", "BA", "BG", "HR", "DK", "ES", "EE", "FI", "FR", "FX", "GB", "GR", "GG", "HU", "IM", "IE", "IT", "JE", "LV", "LI", "LT", "LU", "MC", "NO", "NL", "PL", "PT", "CZ", "RO", "RS", "SK", "SI", "SE", "CH");
                $isoep = array("D", "AND", "A", "B", "BA", "BG", "CRO", "DK", "E", "EST", "SF", "F", "F", "GB", "GR", "GG", "H", "IM", "IRL", "I", "JE", "LET", "LIE", "LIT", "L", "F", "N", "NL", "PL", "P", "CZ", "RO", "RS", "SK", "SLO", "S", "CH");
                ...
                if (strpos($order_info['shipping_code'], 'dpdfrrelais') !== false){
                    ...
                    $retour_option = ($this->config->get('dpdfrrelais_retour') == 0 ? '' : $this->config->get('dpdfrrelais_retour'));
                    ...
                    $retour_option = ($this->config->get('dpdfrpredict_retour') == 0 ? '' : $this->config->get('dpdfrpredict_retour'));
                }else{
                    $service = 'Classic';
                    ...
                    $retour_option = ($this->config->get('dpdfrclassic_retour') == 0 ? '' : $this->config->get('dpdfrclassic_retour'));
                }
                $weight = $weights[$order_id]*100;
                $record->add($order_id, 0, 35);                                                                     //  Référence client N°1 - Id Commande
                $record->add(str_pad((int)$weight, 8, '0', STR_PAD_LEFT), 37, 8);                                   //  Poids du colis sur 8 caractères
                if($service !== 'Relais')
    ...
    protected function getList()
    {
        ...
        foreach ($results as $result)
        {
            ...
            if (strpos($result['shipping_code'], 'dpdfrrelais') !== false){
                ...
                if ($shipping_iso_code_2 == 'FR' || $shipping_iso_code_2 == 'FX' || $shipping_iso_code_2 == 'MC'){
                    $service = 'Classic';
