	...
	public function addOrderHistory($order_id, $data) {
		...
		if ($this->config->get('config_complete_status_id') == $data['order_status_id']) {
      	...
      	if ($data['notify']) {
			...
			// $this->load->language($order_info['language_filename']);
			$this->load->language('sale/dpdfrance');

			$subject = sprintf($this->language->get('text_subject'), $order_info['store_name'], $order_id);

			$message  = $this->language->get('text_order') . ' ' . $order_id . "\n";
			$message .= $this->language->get('text_date_added') . ' ' . date($this->language->get('date_format_short'), strtotime($order_info['date_added'])) . "\n\n";
			
			$order_status_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_status WHERE order_status_id = '" . (int)$data['order_status_id'] . "' AND language_id = '" . (int)$order_info['language_id'] . "'");
				
			if ($order_status_query->num_rows) {
				$message .= $this->language->get('text_order_status') . "\n";
